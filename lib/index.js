...
(function() {
  var Db, EasyMongo, Server, ensureObjectId, isFunction, isObject, mongodb, normalizeArguments, ReplSet;

  mongodb = require('mongodb');

  Db = mongodb.Db, Server = mongodb.Server, ReplSet = mongodb.ReplSet;
  ...
  EasyMongo = (function() {
    ...
    EasyMongo.prototype.getInstance = function(table, after) {
      ...
      if (this.db !== null && this.db.state && this.db.state === 'connected') {
        ...
        //   safe: true
        // });
        if (this.options.cluster == null || this.options.cluster == false) {
          instance = new Db(this.options.db, new Server(this.options.host, this.options.port, {
            auto_reconnect: true
          }));
        } else {          
          var repl_servers = [];
          var repl = this.options.replset;
          for(var i=0; i<repl.length; i++) {              
              repl_servers.push(new Server(repl[i].host,repl[i].port));
          }
          instance = new Db(this.options.db, new ReplSet( repl_servers, {rs_name: this.options.rsname, read_secondary: true} ), {safe: true});
        }
